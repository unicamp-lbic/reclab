...
'''
split_fname_prefix = exp_db.get_arg_val(EXP_ID, 'split_fname_prefix')
...
for fold in range(conf.nfolds):
    ...
    if args.action == 'train':
        ...
        if conf.is_MF:
            MF_file_prefix = exp_db.get_fold_arg_val(EXP_ID, fold, 'MF_file_prefix')
            if MF_file_prefix is None:
                MF_file_prefix = FOLD_PATH
                evalu.gen_mf(split, MF_file_prefix, conf.MFclass, conf.MFargs)
            exp_db.set_fold_arg_val(EXP_ID, fold, 'MF_file_prefix', MF_file_prefix)
            RS = evalu.load_mf(MF_file_prefix, RS)
        # train and save
        train_file_prefix = exp_db.get_fold_arg_val(EXP_ID, fold, 'train_file_prefix')
